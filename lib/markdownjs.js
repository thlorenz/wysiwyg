...
var Dialect = md.Markdown.dialects.Gruber;

var _atxHeader = Dialect.block.atxHeader;
Dialect.block.atxHeader = function (block, next) {
  blocks.push(block);
  return _atxHeader.apply(this, arguments);
}
...
Mapper.prototype._update = function (markdown) {
  ...
  var tree = md.parse(markdown, Dialect);
...
inspect(mapper._markdown)
inspect(mapper._raw)
inspect(mapper._rawParts)

blocks = blocks.map(function (x) {
  return x.replace(/^Markdown\.mk_block\( /, '');
});
