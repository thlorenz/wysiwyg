...
// todo: ensure to clean all marks with no content in between regularly
...
proto.remove = function (start, end) {
  ...
  // todo: if all items between a mark were removed remove the mark
  //       make mark pending first, but remove all pending marks on subsequent removes
...
proto._updateMarks = function (type, mark, range) {
  ...
  if (!existingStart) {
    ...
    // turn mark off
    marks.splice(existingStart.arrIndex, 1);
    // assuming that the buddy is always after the start mark
    if (existingStart.buddy) marks.splice(existingStart.buddy.arrIndex - 1, 1);
  }
...
proto.insertMark = function (type, start, end, mark, surround) {
  ...
  if (range.start === range.end) return this._deferMark(type, range, mark, surround);
...
proto._deferMark = function (type, range, mark, surround) {
  this._pendingMarks.push({ type: type, range: range, mark: mark });
}
